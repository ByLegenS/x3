...
"A key belonging to another kind" is weighed against **one** table of who owns
which field — for `deps`, per matcher, so `pattern` is a `literal` field and
`except` an `import` one. A misspelt key was always refused; a real key on the
