...
**Nor does the blank comment line a directive forces.** `gofmt` separates a
directive from the prose above it with a bare `//`, so writing the line below
costs the line above it too:

```go
// A declaration with ten lines of prose above its example …
//
//x3:case: in=(2) out=4
```

Counted, that separator lowered the practical ceiling by one line wherever an
example was written — an invisible tax, paid by deleting the example. The
formatter writes it, so it belongs to the directive: only a separator
**touching** one is dropped, and a paragraph break in prose still counts.
